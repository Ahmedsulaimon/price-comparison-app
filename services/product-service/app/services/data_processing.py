...
class PriceService:
    ...
    def normalize_unit_price(unit_str):
        ...
        try:
            ...
            patterns = [
            # New patterns added first
            (r'([\d.]+)p\s*/\s*ea', True),               # "20p/ea"
            (r'([\d.]+)p\s*per\s*1\s*unit', True),       # "20p per 1 unit"
            (r'£([\d.]+)\s*per\s*1\s*([\w]+)', False),   # "£1.25 per 1 kilogram"
            ...
            (r'([\d.]+)\s*per\s*([\w/]+)', False),       # 1.45 per litre
            (r'([\d.]+)p\s*([\w/]+)', True),             # "20p100ml" (no separator)
            (r'£([\d.]+)\s*([\w/]+)', False)             # "£1.45L" (no separator)
        ]
